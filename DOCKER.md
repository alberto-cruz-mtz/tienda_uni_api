...
Instrucciones para levantar el entorno de desarrollo con Docker Compose y para construir la imagen lista para
producción.

> Documento complementario al [`README.md`](README.md). La explicación de arquitectura, modelo de datos y stack vive
> allí.
...
> El perfil de Spring activo en este entorno es `docker` (ver `src/main/resources/application-docker.yaml`), que lee
> credenciales desde variables de entorno y permite cookies seguras en `false` para trabajar sobre HTTP plano.
...
La API envía correos transaccionales (verificación de cuenta al registrarse). Las credenciales se inyectan desde el
entorno y se montan en `JavaMailSender` vía [
`MailConfiguration`](src/main/java/tienda/uni/api/auth/configuration/MailConfiguration.java).

| Variable         | Descripción                                                                                                                                      | Default (sólo dev/pruebas) | Aplica a perfil         |
|:-----------------|:-------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------|:------------------------|
| `EMAIL_HOST`     | Host del servidor SMTP.                                                                                                                          | `smtp.gmail.com`           | `dev`, `docker`, `prod` |
| `EMAIL_PORT`     | Puerto SMTP.                                                                                                                                     | `587`                      | `dev`, `docker`, `prod` |
| `EMAIL_USERNAME` | Usuario autenticado contra el SMTP (normalmente la cuenta que envía).                                                                            | `username` (sólo `dev`)    | `dev`, `docker`, `prod` |
| `EMAIL_PASSWORD` | Contraseña o app-password del SMTP. Para Gmail usá una [app password](https://myaccount.google.com/apppasswords), no la contraseña de la cuenta. | `password` (sólo `dev`)    | `dev`, `docker`, `prod` |
| `EMAIL_SENDER`   | Dirección `From:` que verán los destinatarios.                                                                                                   | `no-reply@tiendauni.com`   | `dev`, `docker`, `prod` |

##### Gmail como default sólo para pruebas

Los defaults de `EMAIL_HOST` apuntan a **Gmail** a
propósito, para que el flujo de verificación de correo funcione out-of-the-box en desarrollo local sin tener que
contratar ni configurar un proveedor dedicado.

**En staging o producción no uses Gmail como relay transaccional**: tiene límites de envío agresivos, no está pensado
para alto volumen y la cuenta puede ser suspendida. Configurá un proveedor dedicado y apuntá `EMAIL_HOST`, `EMAIL_PORT`,
`EMAIL_USERNAME` y `EMAIL_PASSWORD` a sus endpoints. Opciones comunes:
...
- **Sender** — `smtp.sender.net:587`

Recordá también ajustar `EMAIL_SENDER` a una dirección `From:` que el provider te autorice a usar (suelen exigir un
dominio verificado).
...
`/docker-entrypoint-initdb.d:ro`. Los archivos se ejecutan **alfabéticamente** la primera vez que se inicializa el
...
Solo corren cuando el volumen está vacío. Si cambiás los scripts y querés
re-ejecutarlos, [reseteá el entorno](#resetear-el-entorno).
...
2. **Stage runtime** (`eclipse-temurin:21-jre-alpine`): copia el JAR final y lo ejecuta con `java -jar`. No incluye
   Maven ni el JDK completo.
...
- `tiendauni/api` es el namespace/nombre sugerido; ajustalo a tu registry (`ghcr.io/tu-org/tiendauni-api`,
  `tu-usuario/tiendauni-api`, etc.).
...
Necesitás un PostgreSQL accesible (RDS, Cloud SQL, instancia autoadministrada, etc.) y todas las variables de
`application-prod.yaml`. Ejemplo mínimo:
...
docker run --rm -p 8080:8080 \
  ...
  -e EMAIL_SENDER="no-reply@tiendauni.com" \
  -e FRONTEND_URL="https://tiendauni.example" \
  -e ERROR_URL="https://tiendauni.example/errors" \
  tiendauni/api:0.0.1
```

> En producción el esquema debe estar migrado por un mecanismo externo (Flyway / Liquibase / migración SQL controlada).
> La imagen **no** ejecuta scripts de inicialización automáticamente.
...
- [ ] `EMAIL_*` (host, port, username, password, sender) cargados con credenciales válidas del SMTP; `EMAIL_SENDER` debe
  ser un buzón que el provider te permita usar como `From:`.
